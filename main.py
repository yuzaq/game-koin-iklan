...
class Aplikasi(App):
    def on_start(self):
        # OTOMATIS AKTIFKAN ADMOB BANNER JIKA RUNNING DI HP ANDROID
        if platform == 'android':
            try:
                from kivy_admob import AdMob
                self.admob = AdMob(ADMOB_APP_ID)
                self.admob.show_banner(ADMOB_BANNER_ID)
            except Exception as e:
                print(f"Gagal Inisialisasi AdMob Banner: {e}")
    ...
    def tunggu_iklan(self, lanjut_fungsi, beri_bonus=False):
        ...
        if platform == 'android':
            try:
                from kivy_admob import AdMob
                admob = AdMob(ADMOB_APP_ID)
                admob.load_rewarded_ad(ADMOB_REWARDED_ID)
        ...
        Clock.schedule_once(selesai, 0.8)
